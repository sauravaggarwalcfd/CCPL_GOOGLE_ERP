                      ...
                      <tr><td colSpan={visCols.length+((showThumb||isListMode)?1:0)+1} style={{padding:grp.key!==null?"0 8px":"0",border:"none"}}>
                        ...
                        {visCols.map(col=>{const f=allFields.find(x=>x.col===col);const isAuto=f?.auto||["calc","autocode"].includes(f?.type||"");const v=row[col]||"";
                          ...
                          if(isListMode){
                            ...
                            // Category → chip badge
                            if(isCat) return(<td key={col} style={{padding:"4px 6px"}}><span style={{background:A.al,color:A.a,padding:"1px 6px",borderRadius:3,fontSize:9,fontWeight:800}}>{v}</span></td>);
                            // ID → purple mono
                            if(isID) return(<td key={col} style={{padding:"4px 8px",fontFamily:"monospace",fontSize:9,fontWeight:700,color:"#6d28d9"}}>{v}</td>);
                            // Value → amber mono
                            if(isValue) return(<td key={col} style={{padding:"4px 8px",fontFamily:"monospace",fontSize:10,fontWeight:700,color:"#854d0e",textAlign:"right"}}>{v}</td>);
