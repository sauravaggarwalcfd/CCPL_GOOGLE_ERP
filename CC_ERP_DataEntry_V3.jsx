...
/* ─── TOKENS ─────────────────────────────────────────────────── */
const C = {
  bg:"#f0f2f6", surface:"#ffffff", border:"#e4e8ef",
  text:"#1a2332", sub:"#5a6a7e", muted:"#9aa5b4", faint:"#f4f6f9",
  red:"#CC0000",   redSoft:"#fff0f0",
  orange:"#E8690A", orangeSoft:"#fff5ee",
  purple:"#6c3fc5", purpleSoft:"#f0eeff",
  blue:"#1d6fa4",  blueSoft:"#e8f3fb",
  green:"#15803d", amber:"#b45309", amberSoft:"#fffbeb",
  slate:"#1e293b", slate2:"#253347", slate3:"#2d3f50",
  inputBg:"#f9fafb", inputBgOn:"#ffffff",
};
...
const SECTIONS = [
  { id:"identity", icon:"📋", label:"Article Identity", acc:C.blue,
    fields:[
      { key:"code",      label:"Article Code",  req:true,  type:"code",     mono:true, hint:"4–5 digits + 2 CAPS · e.g. 5249HP" },
      { key:"desc",      label:"Description",   req:true,  type:"textarea", hint:"Full name with construction · max 120 chars" },
      { key:"shortName", label:"Short Name",    req:false, type:"text",     hint:"Max 25 chars — used on barcode & hang tag" },
      { key:"imageLink", label:"Image Link",    req:false, type:"text",     hint:"Google Drive public share URL" },
      { key:"buyerStyle",label:"Buyer Style No",req:false, type:"text" },
    ],
  },
  { id:"details", icon:"👕", label:"Item Details", acc:"#059669",
    fields:[
      { key:"l1Division",label:"L1 Division",   req:false, type:"auto",   auto:true, hint:"Auto-set: Apparel" },
      { key:"l2Category",label:"L2 Category",   req:true,  type:"select", hint:"Drives L3 options, HSN & GST auto-fill",
        opts:["Tops - Polo","Tops - Tee","Sweatshirt","Tracksuit","Bottoms"] },
      { key:"l3Style",   label:"L3 Style",      req:false, type:"select", hint:"Cascades from L2",
  ...
  { id:"fabric", icon:"🧵", label:"Fabric & Colors", acc:C.purple,
    fields:[
      { key:"mainFabric",label:"Main Fabric",   req:false, type:"select", hint:"FK → RM_MASTER_FABRIC",
        fk:"RM_MASTER_FABRIC",
        opts:["RM-FAB-001 — Pique 180GSM 100% Cotton","RM-FAB-002 — French Terry 260GSM 65/35","RM-FAB-003 — Fleece 320GSM 80/20 CVC"] },
      { key:"fabricName",label:"Fabric Name",   req:false, type:"auto",   auto:true, hint:"Auto from Fabric FK" },
      { key:"colorCodes",label:"Color Code(s)", req:false, type:"text",   hint:"FK → COLOR_MASTER · pipe-separated" },
      { key:"sizeRange", label:"Size Range",    req:false, type:"text",   hint:"e.g. S-M-L-XL-XXL" },
    ],
  },
  { id:"pricing", icon:"₹", label:"Pricing & Tax", acc:C.orange,
    fields:[
      { key:"wsp",        label:"W.S.P (Rs)",       req:false, type:"number",  mono:true, hint:"Wholesale price per piece" },
      { key:"mrp",        label:"MRP (Rs)",          req:false, type:"number",  mono:true, hint:"Maximum retail price" },
      { key:"markupPct",  label:"Final Markup %",    req:false, type:"auto",    auto:true, mono:true, hint:"(MRP − WSP) ÷ WSP × 100" },
      { key:"markdownPct",label:"Final Markdown %",  req:false, type:"auto",    auto:true, mono:true, hint:"(MRP − WSP) ÷ MRP × 100" },
      { key:"hsnCode",    label:"HSN Code",          req:false, type:"select",  hint:"FK → HSN_MASTER · auto-suggested from L2",
        fk:"HSN_MASTER", opts:["6105","6109","6110","6112","6103"] },
      { key:"gstPct",     label:"GST %",             req:false, type:"auto",    auto:true, mono:true, hint:"Auto from HSN Master" },
    ],
  },
  { id:"status", icon:"🏷️", label:"Status & Tags", acc:"#64748b",
    fields:[
      ...
      { key:"tags",    label:"Tags",    req:false, type:"text",     hint:"→ TAG_MASTER · comma-separated" },
    ],
  },
];
...
function Inp({ f, val, onChange, onFocus, onBlur, focused, acc }) {
  const base = {
    ...
    transition:"all .15s",
    background: f.auto ? "#f5f0ff" : focused ? C.inputBgOn : val ? C.inputBgOn : C.inputBg,
    color: f.auto ? C.purple : C.text,
    fontWeight: f.auto ? 700 : 500,
    border:`1.5px solid ${f.auto ? C.purple+"22" : focused ? acc : val ? "#c9d3df" : C.border}`,
    boxShadow: focused && !f.auto ? `0 0 0 3px ${acc}18` : "none",
  ...
  return (
    <input type={f.type==="number"?"number":"text"} value={val||""}
      ...
      placeholder={f.type==="code"?"e.g. 5249HP":""} style={base}/>
  );
}
...
function NewViewModal({ existingNames, onSave, onClose, editView=null }) {
  ...
  const handleSave = () => {
    ...
    onSave({ name:t, fields:[...selFields], icon:"📌", color:C.purple, isSystem:false,
...
function Preview({ fd }) {
  ...
  return (
    <div style={{ width:220, flexShrink:0, background:C.slate,
      borderLeft:"1px solid #2d3f50", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      <div style={{ padding:"10px 13px 8px", borderBottom:"1px solid #2d3f50",
        ...
        <span style={{ fontSize:8.5, fontWeight:900, color:"#64748b", letterSpacing:1,
          textTransform:"uppercase", flex:1 }}>Live Preview</span>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:12, display:"flex",
        ...
        <div style={{ height:120, borderRadius:9, background:"linear-gradient(135deg,#253347,#1a2839)",
          ...
          {st && (<div style={{ position:"absolute", top:7, right:7, display:"flex",
              alignItems:"center", gap:3, padding:"2px 7px", borderRadius:10,
          ...
          {fd.code && (<div style={{ position:"absolute", bottom:7, left:8,
        ...
        {(fd.gender||fd.l2Category||fd.l3Style||fd.fitType||fd.season) && (
          <div style={{ display:"flex",flexWrap:"wrap",gap:3 }}>
            {[[fd.gender,"#1e3a5f","#7dd3fc"],[fd.l2Category,"#14291a","#86efac"],[fd.l3Style,"#251c35","#c4b5fd"],
              [fd.fitType,"#2a1e0a","#fcd34d"],[fd.season,"#1a2a3a","#93c5fd"],[fd.neckline,"#1e1e2e","#a5b4fc"],
              [fd.sleeveType,"#1a1a1a","#d4d4d8"]].filter(([v])=>v).map(([lbl,bg,tx],i)=>(
              <span key={i} style={{ fontSize:7.5,fontWeight:700,padding:"2px 6px",borderRadius:8,background:bg,color:tx }}>{lbl}</span>
            ))}
          </div>
        )}
        ...
        {(fd.wsp||fd.mrp) && (
          <div style={{ background:"#1a2030",borderRadius:6,padding:"8px 9px",border:"1px solid #2a3545" }}>
            ...
            <div style={{ display:"flex",gap:8,marginBottom:(fd.hsnCode||fd.gstPct)?6:0 }}>
              {fd.wsp && <div style={{ flex:1 }}><div style={{ fontSize:7.5,color:"#4d6070" }}>WSP</div><div style={{ fontSize:14,fontWeight:900,color:"#e2e8f0",fontFamily:"'IBM Plex Mono',monospace" }}>₹{fd.wsp}</div></div>}
              {fd.mrp && <div style={{ flex:1 }}><div style={{ fontSize:7.5,color:"#4d6070" }}>MRP</div><div style={{ fontSize:14,fontWeight:900,color:"#e2e8f0",fontFamily:"'IBM Plex Mono',monospace" }}>₹{fd.mrp}</div></div>}
              {markup!==null && <div style={{ flex:1 }}><div style={{ fontSize:7.5,color:"#4d6070" }}>Markup</div><div style={{ fontSize:14,fontWeight:900,fontFamily:"'IBM Plex Mono',monospace",color:markup>=80?"#4ade80":markup>=40?"#fcd34d":"#f87171" }}>{markup}%</div></div>}
            </div>
        ...
        {fd.code && (
          <div style={{ background:"#fff",borderRadius:6,padding:"7px 9px",textAlign:"center",border:"1px solid #e2e8f0" }}>
            <div style={{ display:"flex",justifyContent:"center",alignItems:"flex-end",gap:1.5,height:28,marginBottom:3 }}>
              {Array.from({length:26}).map((_,i)=>{const ch=fd.code[i%fd.code.length];const w=ch?.match(/[0-9]/)?((parseInt(ch)%3)+1)*1.5:2.2;
                return <div key={i} style={{ width:w,height:i%4===0?28:i%3===0?22:25,background:C.slate,borderRadius:.5 }}/>;
              })}
            </div>
            <div style={{ fontSize:9,fontFamily:"'IBM Plex Mono',monospace",fontWeight:900,color:C.slate,letterSpacing:2 }}>{fd.code}</div>
        ...
        {!fd.code&&!fd.desc&&!fd.wsp&&(
          <div style={{ padding:"16px 0",textAlign:"center" }}>
            ...
            <div style={{ fontSize:9.5,color:"#3d5166",lineHeight:1.6 }}>Start filling fields<br/>to see a live preview</div>
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  const visibleFieldKeys = useMemo(()=>{
    ...
    if (section&&section!=="all") { const sec=SECTIONS.find(s=>s.id===section); keys=keys.filter(k=>sec?.fields.some(f=>f.key===k)); }
    if (type&&type!=="all") {
      keys=keys.filter(k=>{
    ...
    if (search) { const q=search.toLowerCase(); keys=keys.filter(k=>{ const f=ALL.find(x=>x.key===k); return f&&(f.label.toLowerCase().includes(q)||f.key.toLowerCase().includes(q)); }); }
    return new Set(keys);
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .frow{transition:background .12s}.frow:hover{background:#f5f7fb}
      ...
      <div style={{ display:"flex", flexDirection:"column", height:"100vh",
        ...
        <div style={{ height:44, flexShrink:0, background:C.slate,
          ...
          <div style={{ display:"flex",alignItems:"center",gap:6,padding:"3px 9px",
            borderRadius:20,background:"#253347",border:"1px solid #2d3f50" }}>
        ...
        <div style={{ background:C.surface, borderBottom:`2px solid ${C.border}`,
          ...
          {/* View pills — system + custom combined */}
          {allViews.map(v => {
            const isActive = activeViewId === v.id;
            return (
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"10px 12px",
            ...
            {reqLeft.length > 0 && (
              <div style={{ display:"flex", alignItems:"center", gap:6,
                ...
                flexShrink:0, animation:shake?"shake .5s ease":"none",
                flexWrap:"wrap" }}>
            ...
            {SECTIONS.map(sec => {
              ...
              const miss = secVisible.filter(f=>f.req&&!f.auto&&!fd[f.key]).length;
                ...
                <div key={sec.id} style={{
                  ...
                  <button onClick={()=>toggle(sec.id)} style={{
                    ...
                    <div style={{ flex:1, textAlign:"left" }}>
                      <div style={{ fontSize:11,fontWeight:900,color:isOn?sec.acc:C.text,lineHeight:1 }}>{sec.label}</div>
                      <div style={{ fontSize:8.5,color:C.muted,marginTop:1 }}>
                          ...
                          <div key={f.key} className="frow" style={{
                            ...
                            <div style={{ padding:f.type==="textarea"?"9px 12px 9px 14px":"7px 12px 7px 14px",
                              ...
                              <div style={{ display:"flex",alignItems:"center",gap:5,flexWrap:"wrap" }}>
                                ...
                                {f.auto&&<span style={{ fontSize:7.5,padding:"1px 5px",borderRadius:3,
                                  background:"#ede9fe",color:C.purple,fontWeight:900,
                                  fontFamily:"'IBM Plex Mono',monospace" }}>← AUTO</span>}
                                {!f.auto&&f.fk&&<span style={{ fontSize:7.5,padding:"1px 5px",borderRadius:3,
                                  background:"#e0f0fb",color:C.blue,fontWeight:900,
                                  fontFamily:"'IBM Plex Mono',monospace" }}>→ FK</span>}
                                {!f.auto&&!f.fk&&f.req&&<span style={{ fontSize:7.5,padding:"1px 5px",borderRadius:3,
                              ...
                              <div style={{ fontSize:8,fontFamily:"'IBM Plex Mono',monospace",
                                color:isFoc?sec.acc+"99":"#c9d3df",marginTop:1,transition:"color .15s" }}>
                                {f.key}
                              </div>
                            </div>
                            {/* Input */}
                            <div style={{ padding:f.type==="textarea"?"7px 14px 7px 0":"5px 14px 5px 0" }}>
                              {showHint&&<div style={{ fontSize:8.5,color:isFoc?C.sub:C.muted,
        ...
        {/* ══ FOOTER ══ */}
        <div style={{ flexShrink:0, background:C.surface, borderTop:`1.5px solid ${C.border}`,
            ...
            {SECTIONS.map(s => {
              ...
              return (
                <div key={s.id} title={s.label} style={{
                  height:4, width:open[s.id]?20:12, borderRadius:2,
          ...
          {ok && <span style={{ fontSize:10,color:C.green,fontWeight:900,animation:"fade .3s ease" }}>
            ✓ Saved to ARTICLE_MASTER
          </span>}
          <div style={{ flex:1 }}/>
          <button onClick={()=>setFd({status:"Active"})} style={{ padding:"6px 13px",
            border:`1.5px solid ${C.border}`, borderRadius:6,
            background:C.faint, color:C.sub, fontSize:10, fontWeight:800, cursor:"pointer" }}>
            ↺ Clear
          </button>
          <button onClick={handleSave} style={{ padding:"6px 20px", border:"none", borderRadius:6,
            background:reqLeft.length>0?"#94a3b8":C.red, color:"#fff",
            fontSize:10, fontWeight:900, cursor:"pointer",
            animation:shake?"shake .5s ease":"none", transition:"background .2s" }}>
